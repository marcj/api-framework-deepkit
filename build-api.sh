...
npm run bootstrap:ci -- --scope @deepkit/angular-universal --scope @deepkit/core --scope @deepkit/core-rxjs --scope @deepkit/app --scope @deepkit/broker \
  ...
  --scope @deepkit/stopwatch --scope @deepkit/template --scope @deepkit/topsort --scope @deepkit/type --scope @deepkit/type-spec  --scope @deepkit/type-compiler  \
  --scope @deepkit/type-angular --scope @deepkit/workflow --scope @deepkit/create-app
npm run link;
npm run tsc;
NODE_OPTIONS=--max_old_space_size=12096 ./node_modules/.bin/typedoc packages/*/index.ts;
cd -;
cp -r /tmp/deepkit-framework-docs/docs/* docs/;
